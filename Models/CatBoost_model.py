...
def run_CatBoost(train_norm, test_norm, hidden_norm):
  ...
  selected_features_catboost = ['GR', 'NPHI_COMB', 'DTC', 'DTS_COMB', 'RHOB', 'Y_LOC', 'GROUP_encoded',
  ...
  for (train_index, test_index) in kf.split(x_train, y_train):
    ...
    catboost_model1.fit(X_train, Y_train.values.ravel(), early_stopping_rounds=100, eval_set=[(X_test, Y_test)], verbose=100)
